...
3.  **체크포인트:** 작업이 완료되거나 안정적인 상태에 도달하면 `checkpoint("Completed Login Implementation")`를 호출하세요.
      * *효과:* 지저분한 `[AUTO-TRJ]` 스냅샷들을 당신이 작성한 메시지와 함께 하나의 깔끔한 커밋으로 압축(Squash)합니다.
      * *장점:* 작업 단계에서의 "생각 과정"은 보존하면서 히스토리는 깔끔하게 유지합니다.
      * **⚠️ 모범 사례:** 기능 구현이 완전히 **완료되었을 때만** 체크포인트를 만드세요. 디버깅 중에 체크포인트를 만들면, 무엇을 시도했고 실패했는지에 대한 상세한 기록을 잃게 됩니다.
      * **참고:** 체크포인트는 메인 프로젝트의 Git 기록에 커밋을 생성하지 **않습니다**. 오직 섀도우 히스토리만 정리합니다.
...
변경을 가하기 전에 코드베이스를 이해하기 위해 이 도구들을 사용하세요:
  ...
  * **`get_global_trajectory(limit=20, since_checkpoint=False)`**: 내 변경 사항이 다른 파일에 파급 효과를 일으키는지 확인할 때 호출하세요. `since_checkpoint=True`를 사용하면 마지막 체크포인트 이후의 모든 변경 사항을 확인할 수 있습니다.
...
4.  **Agent:** `checkpoint("Refactored auth logic to use JWT")` (JWT 사용으로 인증 로직 리팩토링 완료)
      * *서버:* 스냅샷들을 압축 -> `[CHECKPOINT] Refactored auth logic to use JWT` 커밋 생성.
